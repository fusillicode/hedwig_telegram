defmodule Hedwig.Adapters.Telegram do
  ...
  alias HedwigTelegram.{Connection, RTM}
  ...
  def handle_cast({:send, msg}, %{conn: conn} = state) do
    Connection.ws_send(conn, telegram_message(msg))
    {:noreply, state}
  end
  ...
  def handle_cast({:emote, %{text: _text} = msg}, %{conn: conn} = state) do
    Connection.ws_send(conn, telegram_message(msg, %{subtype: "me_message"}))
    {:noreply, state}
  end
  ...
  defp telegram_message(%Hedwig.Message{} = msg, overrides \\ %{}) do
